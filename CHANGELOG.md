...
- Шаблонизация чисел для динамических строк (`Wood 34/50` -> `Wood [[GATNUM:0]]/[[GATNUM:1]]`): шаблон переводится один раз и сохраняется в кэш. Отдельный синтаксис не пересекается с игровыми `{0}`.
- Список окончательных отказов хранится отдельно по версии промпта и модели. В него попадают только повторяющиеся нарушения структуры перевода; временно неудачные ответы можно повторить после перезапуска.
- Защита восстановления changelog отключает модуль после повторных ошибок или постоянного перезаписывания окна и не засоряет лог.
