...
Автоматический перевод английских строк модов Valheim на русский язык. Мод работает на клиенте: перехватывает текст локализации и интерфейса, отправляет его в настроенный OpenAI-совместимый сервис и сохраняет принятые переводы в локальном кэше. **Версия 0.1.4 экспериментальная:** часть строк может остаться на английском.
